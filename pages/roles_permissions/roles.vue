<template>
  <main v-if="hasViewPermission">
            ...
            <table class="table">
              <thead>
                <tr>
                  ...
                  <th>User</th>
                  <th>Role</th>
                  <th v-if="hasEditPermission || hasDeletePermission">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody>
                <template v-if="userDatas.length > 0">
                  <tr v-for="userData in userDatas" :key="userData.id">
                    ...
                    <td class="col3">
                      <span v-if="userData.role_rlt">{{
                        userData.role_rlt.roles.title
                      }}</span>
    ...
    <CommonModalsBaseModal
      :showModal="showRoleAssignModal"
      :pageTitle="pageTitle"
      :modalSize="modalSizeRP"
      :isCreate="isCreateRP"
      :isEdit="isEditRP"
      @createRecord="handleEdit"
      @updateRecord="handleEdit"
      @closeModal="handleCloseFormModal"
    >
...
const guardsToBeChecked =
  "roles_management_view,roles_management_create,roles_management_edit,roles_management_delete";
...
async function fetchUser() {
  try {
    const response = await useMyFetch("admin/user/fetchData", {
      params: {
...
async function fetchData() {
  try {
    const response = await useMyFetch("admin/role/fetchData", {
      params: {
        page: currentPage.value,
        perPage: perPage.value,
      },
...
function handleEdit(id) {
  Swal.fire({
    ...
    if (result.value) {
      try {
        const response = await useMyFetch("admin/rolesUser/" + id + "/edit");
        if (response) {
